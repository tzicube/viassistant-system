...
def _split_commit_by_punct(draft_raw: str) -> tuple[int, str, str]:
    ...
    remain = draft_raw[cut_idx:].lstrip()
    return cut_idx, commit_raw, remain
...
def _words(s: str) -> list[str]:
    return _norm_space(s).strip().split()


def _strip_overlap(prev_text: str, new_text: str, max_k: int = 14) -> str:
    a = _words(prev_text)
    b = _words(new_text)
    if not a or not b:
        return new_text
...
def _too_similar(a: str, b: str) -> bool:
    a = _norm_space(a).strip()
    b = _norm_space(b).strip()
    if not a or not b:
        return False
    if a == b:
        return True
    if len(a) >= 16 and a in b:
        return True
    if len(b) >= 16 and b in a:
        return True
    return False
...
class ViRecordConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def _flush_draft_commit(self):
        full_raw = (self.mem.stt_cumulative or "").strip()
        cur = max(0, min(self.mem._stt_committed_len, len(full_raw)))
        draft_raw = full_raw[cur:].strip()
        if draft_raw:
    ...
    async def _line1_stt(self):
        ...
        while not self.mem.stopped:
            ...
            if streamer.ready():
                ...
                if cut_idx > 0 and len(_norm_space(commit_raw)) >= MIN_COMMIT_CHARS:
    ...
    async def _pause_commit_loop(self):
        ...
        while not self.mem.stopped:
            ...
            idle = time.time() - self.mem.last_stt_update_ts
            if idle >= PAUSE_SEC and len(_norm_space(draft_raw)) >= MIN_COMMIT_CHARS:
                self.mem._stt_committed_len = len(full_raw)
                await self._commit_source_segment(draft_raw)
    ...
    async def _line2_translate_commits(self):
        logger.warning("[line2] start translate")
        ...
        while not self.mem.stopped:
            ...
            async for chunk in stream_translate_segment_async(
                ...
                seg_cum += (chunk or "")
                delta = seg_cum[len(prev_emit):] if seg_cum.startswith(prev_emit) else seg_cum
                if delta:
    ...
    async def _on_stop(self):
        ...
        while time.time() - t0 < 2.0:
            if self.commit_q.empty() and not self.mem._translating:
                break
